...
pub enum SifisError {
    ...
    MutexError(),
    #[error("Thesholds must be only 4 in this order -t SIFIS_PLAIN, SIFIS_QUANTIZED, CRAP, SKUNK")]
    ThesholdsError(),
}

impl From<std::io::Error> for SifisError {
    fn from(_item: std::io::Error) -> Self {
        SifisError::WrongFile()
    }
}

impl From<serde_json::Error> for SifisError {
    fn from(_item: serde_json::Error) -> Self {
        SifisError::WrongJSONFile()
    }
}

impl From<csv::Error> for SifisError {
    fn from(_item: csv::Error) -> Self {
        SifisError::WrintingError()
    }
}

impl<T> From<PoisonError<MutexGuard<'_, T>>> for SifisError {
    fn from(_item: PoisonError<MutexGuard<'_, T>>) -> Self {
        SifisError::MutexError()
    }
}

impl From<Box<dyn std::any::Any + Send>> for SifisError {
    fn from(_item: Box<dyn std::any::Any + Send>) -> Self {
        SifisError::ConcurrentError()
    }
}
...
pub(crate) fn read_files(files_path: &Path) -> Result<Vec<String>, SifisError> {
    ...
    while let Some(path) = stack.pop() {
        if path.is_dir() {
            ...
            paths.try_for_each(|p| -> Result<(), SifisError> {
                let pa = p?.path();
                stack.push(pa);
                Ok(())
            })?;
            ...
            if ext.is_some() && check_ext(ext.ok_or(SifisError::PathConversionError())?) {
...
pub(crate) fn read_json(
    ...
    let vec = val["source_files"]
        .as_array()
        .ok_or(SifisError::ReadingJSONError())?;
    let mut covs = HashMap::<String, Vec<Value>>::new();
    vec.iter().try_for_each(|x| -> Result<(), SifisError> {
        let name = Path::new(prefix).join(
            x["name"]
                .as_str()
...
pub(crate) fn read_json_covdir(
    ...
    while let Some((val, prefix)) = stack.pop() {
        val.iter()
            .try_for_each(|(key, value)| -> Result<(), SifisError> {
...
pub(crate) fn get_coverage_perc(covs: &[Value]) -> Result<f64, SifisError> {
    ...
    let (tot_lines, covered_lines) =
        covs.iter()
            .try_fold((0., 0.), |acc, line| -> Result<(f64, f64), SifisError> {
...
pub(crate) fn get_root(path: &Path) -> Result<FuncSpace, SifisError> {
    ...
    let lang = guess_language(&data, path)
        .0
        .ok_or(SifisError::LanguageError())?;
    debug!("{:?} is written in {:?}", path, lang);
    let root = get_function_spaces(&lang, data, path, None).ok_or(SifisError::MetricsError())?;
    Ok(root)
}

// Check complexity of a metric
// Return true if al least one metric exeece a threshold , false otherwise
#[inline(always)]
